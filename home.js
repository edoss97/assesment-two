...
const canWeDeliver = zip => {
    for (let i = 0; i < deliveryAreaZipCodes.length; i++) {
        if (zip === deliveryAreaZipCodes[i]) { return "You're in our delivery zone!"
        } 
    }
...
const canWeDeliverTwo = zip => {
    if (deliveryAreaZipCodes.includes(zip)) { return "You're in our delivery zone!"
    } 
    return `Sorry, we can't deliver to that address`
}
...
const replace15 = (newArr) => {
for (let i = 0; i < newArr.length; i++) {
    newArr.replace("15","10");
	}
	return newArr
	}


    let updatedDeals = deals.splice(0,deals.length)
    console.log(updatedDeals)
 console.log(replace15(updatedDeals))

//const  updatedDeals = title.replace("15", "10")
